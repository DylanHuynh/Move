...
const {generateMoveTitlePrompt} = require('./prompts')
...
async function getMoveTitle(eventDescription) {
  const body = `{"model": "${MODEL}", "prompt": "${"Description: " + eventDescription + ". " + generateMoveTitlePrompt}", "stop": "."}`
  console.log( `"${"Description: " + eventDescription + ". " + generateMoveTitlePrompt}"`);
